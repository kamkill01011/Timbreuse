...
public class TeacherFilter implements Filter {
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException, ServletException {
		...
		else if(((User) session.getAttribute(ATT_SESSION_USER)).getPermissionLevel() == 2) chain.doFilter(request, response);
		else request.getRequestDispatcher(VIEW_STUDENT).forward(request, response);
	}
